...
function dictValueParserTonA$Data(): DictionaryValue<TonA$Data> {
 ...
 type TicketNftItem_init_args = {
    $$type: 'TicketNftItem_init_args';
    collection_address: Address;
    item_index: bigint;
}

function initTicketNftItem_init_args(src: TicketNftItem_init_args) {
    return (builder: Builder) => {
        let b_0 = builder;
        b_0.storeAddress(src.collection_address);
        b_0.storeInt(src.item_index, 257);
...
const TicketNftItem_types: ABIType[] = [
...
const TicketNftItem_getters: ABIGetter[] = [
    {"name":"get_nft_data","arguments":[],"returnType":{"kind":"simple","type":"GetNftData","optional":false}},
]

export const TicketNftItem_getterMapping: { [key: string]: string } = {
    'get_nft_data': 'getGetNftData',
}

const TicketNftItem_receivers: ABIReceiver[] = [
    {"receiver":"internal","message":{"kind":"typed","type":"Transfer"}},
    {"receiver":"internal","message":{"kind":"typed","type":"GetStaticData"}},
]

export class TicketNftItem implements Contract {
    
    static async init(collection_address: Address, item_index: bigint) {
        return await TicketNftItem_init(collection_address, item_index);
    }
    
    static async fromInit(collection_address: Address, item_index: bigint) {
        const init = await TicketNftItem_init(collection_address, item_index);
        const address = contractAddress(0, init);
        return new TicketNftItem(address, init);
    }
    
    static fromAddress(address: Address) {
        return new TicketNftItem(address);
    }
    
    readonly address: Address; 
    readonly init?: { code: Cell, data: Cell };
    readonly abi: ContractABI = {
        types:  TicketNftItem_types,
        getters: TicketNftItem_getters,
        receivers: TicketNftItem_receivers,
        errors: TicketNftItem_errors,
    };
